...
<Row>
  ...
  <CodeGroup title="Request" tag="GET" label="/meta" targetCode={`curl -X GET '${props.appDetail.api_base_url}/meta' \\\n-H 'Authorization: Bearer {api_key}'`}>
    ...
    {
      "tool_icons": {
        "dalle2": "https://cloud.devagentic.io/console/api/workspaces/current/tool-provider/builtin/dalle/icon",
        "api_tool": {
          "background": "#252525",
          "content": "\ud83d\ude01"
        }
...
<Row>
  ...
  <Col sticky>
    ...
    >
    ```bash {{ title: 'cURL' }}
    curl --location --request POST 'https://api.devagentic.io/v1/apps/annotation-reply/{action}' \
    --header 'Authorization: Bearer {api_key}' \
    --header 'Content-Type: application/json' \
    --data-raw '{
        "score_threshold": 0.9,
