...
class BillingConfig(BaseSettings):
    """
    Configuration for platform billing features and Stripe integration
    """
    ...
    STRIPE_API_KEY: Optional[str] = Field(
        description="Stripe secret API key for server-side operations (e.g., sk_test_... or sk_live_...)",
        default=None,
    )

    STRIPE_PUBLISHABLE_KEY: Optional[str] = Field(
        description="Stripe publishable API key for client-side operations (e.g., pk_test_... or pk_live_...)",
        default=None,
    )

    # Stripe Webhook Configuration
    STRIPE_WEBHOOK_SECRET: Optional[str] = Field(
        description="Stripe webhook endpoint secret for verifying webhook authenticity",
        default=None,
    )

    # Stripe Price IDs for different plans and billing intervals
    STRIPE_PROFESSIONAL_MONTHLY_PRICE_ID: Optional[str] = Field(
        description="Stripe Price ID for Professional plan monthly billing",
        default=None,
    )

    STRIPE_PROFESSIONAL_YEARLY_PRICE_ID: Optional[str] = Field(
        description="Stripe Price ID for Professional plan yearly billing",
        default=None,
    )

    STRIPE_TEAM_MONTHLY_PRICE_ID: Optional[str] = Field(
        description="Stripe Price ID for Team plan monthly billing",
        default=None,
    )

    STRIPE_TEAM_YEARLY_PRICE_ID: Optional[str] = Field(
        description="Stripe Price ID for Team plan yearly billing",
        default=None,
    )

    # Legacy price ID for backward compatibility
    STRIPE_CORE_PLAN_PRICE_ID: Optional[str] = Field(
        description="Legacy Stripe Price ID for Core Capabilities plan (deprecated)",
        default=None,
    )

    # Application URLs for Stripe redirects
    NEXT_PUBLIC_APP_URL: Optional[str] = Field(
        description="Base URL of the application for Stripe redirect URLs",
    ...
    def is_stripe_configured(self) -> bool:
        ...
        return bool(
            self.STRIPE_API_KEY and
            self.STRIPE_PUBLISHABLE_KEY and
            self.STRIPE_WEBHOOK_SECRET and
            (
                # Check if at least one price ID is configured
                self.STRIPE_PROFESSIONAL_MONTHLY_PRICE_ID or
                self.STRIPE_PROFESSIONAL_YEARLY_PRICE_ID or
                self.STRIPE_TEAM_MONTHLY_PRICE_ID or
                self.STRIPE_TEAM_YEARLY_PRICE_ID or
                self.STRIPE_CORE_PLAN_PRICE_ID  # Legacy fallback
    )
        )

    def get_price_id(self, plan: str, interval: str) -> Optional[str]:
        """Get the appropriate Stripe Price ID for a plan and billing interval"""
        price_map = {
            ("professional", "month"): self.STRIPE_PROFESSIONAL_MONTHLY_PRICE_ID,
            ("professional", "year"): self.STRIPE_PROFESSIONAL_YEARLY_PRICE_ID,
            ("team", "month"): self.STRIPE_TEAM_MONTHLY_PRICE_ID,
            ("team", "year"): self.STRIPE_TEAM_YEARLY_PRICE_ID,
        }

        price_id = price_map.get((plan, interval))

        # Fallback to legacy price ID if specific one not found
        if not price_id and self.STRIPE_CORE_PLAN_PRICE_ID:
            price_id = self.STRIPE_CORE_PLAN_PRICE_ID
