...
class FeatureService:
    ...
    def get_knowledge_rate_limit(cls, tenant_id: str):
        ...
        if dify_config.BILLING_ENABLED and tenant_id:
            ...
            # Use default values for our Stripe billing system
            knowledge_rate_limit.limit = 50  # Default limit for free plan
            knowledge_rate_limit.subscription_plan = "sandbox"
    ...
    def _fulfill_params_from_billing_api(cls, features: FeatureModel, tenant_id: str):
        # Use our Stripe billing system instead of external billing API
        try:
            from services.stripe_service import StripeService

            # Check if Stripe is configured
            if not StripeService.is_configured():
                # If Stripe is not configured, use sandbox defaults
                features.billing.enabled = True
                features.billing.subscription.plan = "sandbox"
                features.billing.subscription.interval = "month"
                return

            # Get billing info from our Stripe service
            stripe_service = StripeService()
            billing_info = stripe_service.get_billing_info(tenant_id)

            # Enable billing features
            features.billing.enabled = True

            # Set subscription plan based on Stripe status
            if billing_info.get("is_active", False):
                features.billing.subscription.plan = "core"
                features.billing.subscription.interval = "month"
                features.webapp_copyright_enabled = True
            else:
                features.billing.subscription.plan = "sandbox"
            ...
            # Set default limits for our billing system
            if features.billing.subscription.plan == "sandbox":
                features.members.limit = 1
                features.apps.limit = 10
                features.vector_space.limit = 5
                features.documents_upload_quota.limit = 50
                features.annotation_quota_limit.limit = 10
            else:
                features.members.limit = 10
                features.apps.limit = 50
                features.vector_space.limit = 100
                features.documents_upload_quota.limit = 500
                features.annotation_quota_limit.limit = 100
                features.can_replace_logo = True

        except Exception as e:
            # If there's any error with Stripe service, fallback to enabled billing with sandbox plan
            features.billing.enabled = True
            features.billing.subscription.plan = "sandbox"
            features.billing.subscription.interval = "month"
    ...
    def _fulfill_params_from_enterprise(cls, features: SystemFeatureModel):
        ...
        if "License" in enterprise_info:
            ...
            if "status" in license_info:
                status_value = license_info.get("status", "inactive")
                try:
                    features.license.status = LicenseStatus(status_value)
                except ValueError:
                    features.license.status = LicenseStatus.INACTIVE
        ...
        if "PluginInstallationPermission" in enterprise_info:
            ...
            scope_value = plugin_installation_info.get("pluginInstallationScope", "all")
            try:
                features.plugin_installation_permission.plugin_installation_scope = PluginInstallationScope(scope_value)
            except ValueError:
                features.plugin_installation_permission.plugin_installation_scope = PluginInstallationScope.ALL
            features.plugin_installation_permission.restrict_to_marketplace_only = plugin_installation_info.get(
